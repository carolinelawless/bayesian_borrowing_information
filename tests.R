...
for(i in 1:nrep){
  ...
  # --- Run model ---
  model <- jags.model(textConnection(model_string_indep), data = data_jags, n.chains = 3, n.adapt = 1000)
  update(model, 1000)
...
#  if(quantile(samples_mat[, "mu"], 0.9975) < 2 | quantile(samples_mat[, "mu"], 0.0025) > 2){
  if(quantile(samples_mat[, "mu"], 0.0025) > 2){
    print("reject")
    power <- power + 1
  }else{print("accept")}
}
...
c(mean(samples_mat[, "alpha[1]"]), mean(samples_mat[, "alpha[2]"]), mean(samples_mat[, "alpha[3]"]))
