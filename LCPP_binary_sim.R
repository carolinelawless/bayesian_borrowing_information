...
model {
  ...
  # Each transition can have its own precision tau_theta[j]
  for (j in 2:J) {
    theta_hist[j] ~ dnorm(theta_hist[j-1], tau_theta[j-1])
    tau_theta[j-1] <- pow(sigma_theta[j-1], -2)
    sigma_theta[j-1] ~ dunif(0, 5)
  }
